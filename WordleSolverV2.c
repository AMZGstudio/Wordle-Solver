...
#define INPUT_SIZE 7
#define ARRAY_SIZE 6
#define ARRAY_LENGTH 12972
#define PERFER_ARRAY_LEN 4630
...
void getResultAndAsign(char input[7], char blackList[], char greenLetters[], char yellowLetters[])
{
    ...
    for(int i=0; i<5; i++)
    {
        ...
        while(result!='y' && result!='g' && result!='b');
        printf("%c", result);
        results[i] = result;
    }
    printf("\n");
...
void calculateAllowedWords(char array[][6], char blacklist[26], char greenLetters[], char yellowLettersAll[][ARRAY_SIZE])
{
    ...
    {   
        int greenStoppedIt=0;
        int yellowStoppedIt=0;
        int blackStoppedIt=0;
        ...
        for(int g=0; g<5; g++)
        {
            ...
            for(int d=0; d<5; d++)
            {
                if(greenLetters[d]!=' ' && array[i][d]!=greenLetters[d])
                {
                    //printf("we will remove a word on round %d\n", i);
                    greenStoppedIt=1;
                }
            }
            ...
            for(int f=0; f<26; f++)
            {
                ...
                for(int h=0; h<5; h++)
                {
                    ...
                    for(int c=0; c<5; c++)
                    {
                        for(int m=0; m<5; m++)
                        {
                            if(blacklist[f]==yellowLettersAll[c][m])
                            {
                                overrideBlackList=1;
                            }
                        }
                    }
                    if(greenLetters[g]==blacklist[f])
                        overrideBlackList=1;
                }
            ...
            for(int a=0; a<5; a++)
            {
                //cycle through each rounds yellow letters
                for(int x=0; x<5; x++)
                {
                    if(yellowLettersAll[x][a]!=' ' && array[i][a]==yellowLettersAll[x][a])
                        yellowStoppedIt=1;
                    else if(yellowLettersAll[x][a]!=' ' && ((yellowLettersAll[x][a]!=array[i][0]) && (yellowLettersAll[x][a]!=array[i][1]) && (yellowLettersAll[x][a]!=array[i][2]) && (yellowLettersAll[x][a]!=array[i][3]) && (yellowLettersAll[x][a]!=array[i][4])))
                        yellowStoppedIt=2;
                }
            }
        }
        //empty a string because it doesnt follow all the rules
        if(greenStoppedIt==1 || blackStoppedIt==1 || yellowStoppedIt!=0)
        {
            for(int t=0; t<5; t++)
            {
                array[i][t] = '\0';
...
int doATurn(char yellowLetters[], char yellowLettersAll[][ARRAY_SIZE], char blacklist[], char greenLetters[], char array[][ARRAY_SIZE], float priority[], int amountWords, char perferedArray[][ARRAY_SIZE], int round)
{
    int temp=0;
    amountWords=0;
    int highestNumLoc=ARRAY_LENGTH+1, secHighestNumLoc=ARRAY_LENGTH+1, thrdHighestNumLoc=ARRAY_LENGTH+1;
    ...
    for(int i=0; i<ARRAY_LENGTH; i++)
    {
        ...
        if(priority[i]>priority[highestNumLoc] && array[i][0]!='\0')
        {
    ...
    if(amountWords>3)
    {   
        printf("Top picks(bigger number is better): \n-%s %.3f\n", array[highestNumLoc], priority[highestNumLoc], array[thrdHighestNumLoc], priority[thrdHighestNumLoc]);

        if(priority[secHighestNumLoc]!=0)
            printf("-%s %.3f\n", array[secHighestNumLoc], priority[secHighestNumLoc]);
        if(priority[thrdHighestNumLoc]!=0)
    ...
    else
    {
        printf("Top picks(bigger number is better): \n");
        int threeWords=0;
        for(int i=0; i<ARRAY_LENGTH && threeWords<3; i++)
        {
            if(array[i][0]!='\0')
            {
                printf("-%s %.3f\n", array[i], priority[i]);
...
int main()  
{
    ...
    char greenLetters[ARRAY_SIZE] = {' ', ' ', ' ', ' ', ' '};
    char yellowLettersAll[][ARRAY_SIZE] = {{' ', ' ', ' ', ' ', ' '},{' ', ' ', ' ', ' ', ' '},{' ', ' ', ' ', ' ', ' '},{' ', ' ', ' ', ' ', ' '},{' ', ' ', ' ', ' ', ' '}};
    printf("calculating...\n\n");
    calculateAllowedWords(array, blacklist, greenLetters, yellowLettersAll);
